...
## Purpose
The user chose **Session 026** — it now lives at `/` (route file `src/app/page.tsx` renders `SequencerPage` directly; `/sequencer` is a redirect to `/`). The three-door chooser moved to **`/concepts`** (arcade/atlas/sequencer doors; recital/os remain archived routes). Shared facts live in `src/lib/content.ts`.

## Map — chooser doors (`/concepts`) + promoted homepage

| Route | Component | Concept | Signature elements |
|-------|-----------|---------|--------------------|
| `/` (was `/sequencer`) | `components/sequencer/SequencerPage.tsx` | DAW session "session_026", near-black `#0a0a0c`, Silkscreen + JetBrains Mono | ENRICHED v3 (see below) |
...
## SequencerPage v4 — homepage (`/`), i18n + themes
...
## Other concepts still routed (`/concepts` doors)
...
## Map — archived (still routed, not on chooser)
...
## Open Threads
- Sequencer = homepage (v4: drag clips, popover customizer, i18n, light mode). `/concepts` still hosts all studies; recital/os/atlas/arcade files are uncommitted — deletion is permanent if user wants cleanup.
