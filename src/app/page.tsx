...
export default function Home() {
  ...
  const activePt = useMemo(() => {
    return (
      (t.projects as any)[activeProject.id] || {
        name: "",
        description: "",
      }
    );
        ...
        <section id="archive" className="py-20 md:py-28">
          <div className="mx-auto max-w-screen-2xl px-6 md:px-12">
            ...
            <div ref={previewRef} className="mt-10 mb-16">
              {/* Preview Container Wrapper - Flex layout to guarantee no overlap */}
                  ...
                  <div className="flex items-center justify-between border-b border-border bg-background/50 px-4 py-3 backdrop-blur-sm">
                    ...
                    <div className="flex items-center gap-1 rounded-md border border-border bg-background p-0.5">
                      <button
                        ...
                        title={(t.archive as any).desktopView}
                      >
                        <Monitor size={12} />
                      </button>
                      <button
                        ...
                        title={(t.archive as any).mobileView}
                      >
                        <Smartphone size={12} />
                      </button>
                    </div>
                  ...
                  <div className="relative h-[calc(100%-41px)] w-full bg-background overflow-hidden">
                    {interactActive ? (
                      ...
                      >
                        ...
                        <p className="mt-2 max-w-md font-body text-xs text-muted">
                        ...
                        <span className="mt-6 rounded-full bg-text px-4 py-2 font-body text-[10px] uppercase tracking-[0.2em] text-background transition-colors group-hover/iframe:bg-accent">
                          {(t.archive as any).clickToInteract}
                        </span>
                        <p className="mt-3 font-body text-[9px] tracking-wide text-muted/60 opacity-0 transition-opacity duration-300 group-hover/iframe:opacity-100">
                          {(t.archive as any).interactInstructions}
                        </p>
                      </div>
                    )}
                  </div>
              ...
              <div className="mt-8 flex flex-col items-center text-center px-4">
                ...
                <div className="mt-6 flex flex-wrap items-center justify-center gap-4">
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="inline-flex items-center gap-2 rounded-full bg-text px-4 py-2 font-body text-[10px] uppercase tracking-[0.2em] text-background transition-colors hover:bg-accent"
                  >
                    <span>{(t.archive as any).openInNewTab}</span>
                  ...
                  {activeProject.github && (
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className="inline-flex items-center gap-2 rounded-full border border-border bg-surface px-4 py-2 font-body text-[10px] uppercase tracking-[0.2em] text-text transition-colors hover:border-text/30"
                    >
                      <Github size={12} />
                      <span>{(t.archive as any).viewGithub}</span>
                    </a>
                  )}
                </div>
                  ...
                  <div className="flex items-center gap-1.5">
                    {webProjects.map((_, i) => (
                      <button
                        ...
                        title={((t.projects as any)[webProjects[i].id] || {}).name}
                      />
                    ))}
                  </div>
            ...
            {/* Redesigned Complete Archive List */}
            <div className="mt-16 divide-y divide-border overflow-hidden rounded-3xl border border-border bg-surface">
